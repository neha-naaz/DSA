...
the kth highest test score after a new score has been submitted. More specifically, we are looking for the kth highest score in 
...
// Time Complexity - O(mlogk) - where m is the number of times add() is called, and k represents the rank of the largest number to be tracked 
// Space - O(k) since we only store k largest elements in the heap
class KthLargest {
    ...
    public KthLargest(int k, int[] nums) {
        ...
        for (int num : nums) {
            pq.offer(num);
          // if size exceeds k remove pop element
            if(pq.size() > kVal){
                pq.poll();
            }
        }
    ...
    public int add(int val) {
        pq.offer(val);
        if(pq.size()>kVal)pq.poll();
      // return the current kth largest
        return pq.peek();
    }
}
